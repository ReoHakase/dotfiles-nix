...
| `nix build '.#darwinConfigurations.reohakase.system' --no-link` | Apple Silicon macOS runner で system drv をビルド（`switch` はしない）。                        |
| `nix build '.#packages.x86_64-linux.home-reohakuta-kcvl' --dry-run` | Ubuntu runner で Home Manager activation package の実現計画を確認。                          |
...
> [!NOTE] > Linux Home Manager は `llm-agents` / TeX / GUI app を含む閉包が大きく、GitHub-hosted Ubuntu runner の通常 timeout では実ビルドが安定しないため、CI では `drvPath` 評価と `--dry-run` に留める。
