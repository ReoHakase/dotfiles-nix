...
- **nix-darwin の `homebrew`:** `hosts/<hostname>.nix` の `homebrew.enable` で、`brew bundle` による宣言管理が有効になる。例として `wtp` は `satococoa/tap` と `brews` に列挙。`onActivation.cleanup` は **`none`**（Brewfile に無い formula は自動削除しない）。全面を宣言だけにしたい場合は [nix-darwin manual](https://nix-darwin.github.io/nix-darwin/manual/index.html#opt-homebrew.onActivation.cleanup) の `uninstall` / `zap` を検討。
- **`brew` コマンド:** `home.sessionPath` に **`/opt/homebrew/bin`** と **`/opt/homebrew/sbin`** を含めている（Apple Silicon の既定）。Homebrew 本体は従来どおり brew.sh の手順で `/opt/homebrew` に入れる前提。
