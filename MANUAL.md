...
| Home Manager | `home/darwin.nix` / `home/linux.nix` の `home.username` と `home.homeDirectory` |
...
3. `home/*.nix` / `home/modules/*.nix` / `hosts/*.nix` を変えたら必ず上記の **switch** を実行する。

## `config/` の追加・削除・編集（`~/.config` を宣言管理するとき）

`home/modules/*.nix` や OS 別 Home Manager モジュールの `xdg.configFile` が **`../../config/...` / `../config/...` を `source` として** `~/.config` 以下にシンボリックリンクを張る。Nix は **flake の入力として Git の追跡ファイル**を読むため、リポジトリ直下で作業する場合は **未コミットの新規パスがあるとビルドが失敗**することがある（`Path 'config/...' does not exist in Git repository` など）。
...
2. 関連する `home/modules/*.nix` または OS 別 Home Manager モジュールの `xdg.configFile` にエントリを足す。例:
...
1. 該当する Home Manager モジュールから `xdg.configFile` 行を削除する。
...
`~/.config/git/config` の一部は **`programs.git` など Home Manager のモジュール**が生成する。リポジトリに平文の `config/git/config` を置かず、**`home/modules/git.nix` の `programs.git.settings`** を編集する。`config/git/README.md` を参照。
...
- 正本を **`config/` + `home/modules/`** に置く運用なら、**いじるのは基本リポジトリ側**で、`switch` で `~/.config` に反映するのが一番わかりやすい。
...
`flake.nix` に `nix-casks` の input を追加済み。`home/darwin.nix` の `nixCasks` リストに、例として次のようにパッケージを足す（名前は [NixCasks の一覧](https://nix-casks.yorganci.dev/) を参照）:
...
macOS では [NixCasks](#nixcasksgui) や Homebrew cask で GUI を足すことが多いが、**この flake の Ubuntu 向け Home Manager** では次を宣言している（`home/modules/linux/gui-apps.nix`）。
...
| Proton VPN | nixpkgs の `proton-vpn`。macOS では Homebrew cask の `protonvpn` で入れる | `proton-vpn` |
| VeraCrypt | nixpkgs の `veracrypt`。macOS では FUSE-T 版の Homebrew cask `veracrypt-fuse-t` で入れる | `veracrypt` |
...
CLI の `sshfs` は Ubuntu 向けの `home/linux.nix` で nixpkgs の通常版 `sshfs` を入れる。macOS の SSHFS は FUSE-T tap の `sshfs-fuse-t` に寄せる。
...
nix build '.#packages.x86_64-linux.proton-vpn' --no-link
nix build '.#packages.x86_64-linux.veracrypt' --no-link
...
Git の名前・メールアドレスは Home Manager が `home/modules/git.nix` の `programs.git.settings.user` から生成する。署名鍵 ID はホストごとに変わるため、リポジトリには入れず `~/.config/git/local` にだけ書く。
...
`nixd` と `nixfmt` は **`home/modules/packages.nix` の `home.packages`** に含めている。適用後はターミナルで `command -v nixd` が Nix のパスを指すことを確認する。概要は [editor-setup](https://github.com/nix-community/nixd/blob/main/nixd/docs/editor-setup.md)。
...
| 状況                 | 例                                                                                                                                                                                                                                                                                        |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 依存なしで外しやすい | `fzf`, `bat`, `eza`, `fd`, `gh`, `tmux`, `ffmpeg`, `gnupg`, `graphviz`, `hyperfine`, `nmap`, `typst`, `uv`, `wget`, `starship` など（`brew uses --installed` が空なら候補）                                                                                                               |
...
**実施例（メイン Mac）:** 上記のとおり重複 formula を削減。`fastfetch` などは **nixpkgs の `home.packages`** に移し、brew の formula は `cleanup = uninstall` で整理する。
...
`nix` は **Git で追跡されたファイル**を主に flake 入力として見る。未コミットの変更があると警告が出ることがある。本番に近い評価では変更を `git add` / `commit` してから `nix flake check` や `switch` するとよい。特に `home/modules/` などの新規ファイルを追加した直後は、`git add` で追跡に乗せてから評価する。

### actrun（[mizchi/actrun](https://github.com/mizchi/actrun)）

> [!NOTE]
> **NixOS/nixpkgs の公式パッケージ集合にまだ入っていない**ため、このリポジトリでは `flake.nix` の **`inputs.actrun`** と **`inputs.actrun.inputs.moonbit-overlay.overlays.default` → `inputs.actrun.overlays.default`** の順で [actrun の flake](https://github.com/mizchi/actrun) から `pkgs.actrun` を取り込み、`home.packages` に追加している。`actrun` の package は `moonPlatform` を要求するため、actrun overlay の前に MoonBit overlay が必要。nixpkgs 本体への追加をしたい場合は [NixOS/nixpkgs](https://github.com/NixOS/nixpkgs) へパッケージ定義の PR を出す（別作業）。

Nix 検証用 workflow は `actrun workflow run .github/workflows/nix.yml` でローカル検証できる。実 runner と完全一致しない場合は、下のコマンドを直接確認する。

## CI（GitHub Actions）

`.github/workflows/nix.yml` は push / pull request / 手動実行で、Nix 構成を次の粒度で検証する。

| ステップ                                                        | 意味                                                                                           |
| --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `nix develop -c nixfmt --check` / `statix check` / `deadnix --fail` | tracked Nix ファイルの format / lint / dead-code 検証。                                      |
| `nix flake lock --no-update-lock-file`                          | `flake.nix` と `flake.lock` の同期漏れを検出。                                                 |
| `nix eval --raw '.#packages.x86_64-linux.home-reohakuta-kcvl.drvPath'` | Ubuntu runner で Linux Home Manager output を対象指定で評価。                              |
| `nix eval --raw '.#darwinConfigurations.reohakase.system.drvPath'` | Apple Silicon macOS runner で Darwin system output を対象指定で評価。                         |

> [!NOTE] > **`darwin-rebuild switch` と system / Home Manager build は CI では実行しない。** root・activation・nix-homebrew / Homebrew が **そのランナー固有**になり、本番 Mac の再現にもならない。CI は対象 output の評価までに留める。

> [!NOTE] > Linux Home Manager は `llm-agents` / TeX / GUI app を含む閉包が大きく、GitHub-hosted Ubuntu runner の通常 timeout では実ビルドが安定しないため、CI では `drvPath` 評価に留める。

> [!TIP] > Apple Silicon 向け（`aarch64-darwin`）構成は `macos-26` runner でビルドする。GitHub の macOS runner label は移行があるため、失敗時は [GitHub-hosted runners reference](https://docs.github.com/en/actions/reference/runners/github-hosted-runners) を確認する。

### Renovate

`.github/workflows/renovate.yml` は毎日 03:00 JST に self-hosted Renovate を実行する。`.github/renovate.json5` では Nix manager と `lockFileMaintenance` を有効にし、`flake.lock` / GitHub Actions / Vicinae AppImage の固定 hash 更新 PR を作る。Renovate PR は `build(deps): ⬆️ ...` の Conventional Commit + Gitmoji 形式に揃え、CI が通ったあと Renovate 側で automerge する（`platformAutomerge = false`）。

workflow には `RENOVATE_TOKEN` secret を設定しておくと、Renovate が作った PR でも通常の PR / push workflow が動く。未設定時は `GITHUB_TOKEN` に fallback するが、GitHub の制限で後続 CI が発火しない場合がある。
...
| `brew untap` / `cleanup` が失敗                        | nixpkgs に移した formula が brew に残っていると、`brew bundle --cleanup` や untap が失敗することがある。該当 formula を `brew uninstall` してから再適用。                                                                                                                                                                                   |
