...
- **`onActivation.cleanup`（[nix-darwin manual](https://nix-darwin.github.io/nix-darwin/manual/index.html#opt-homebrew.onActivation.cleanup)）:**  
  ...
  - **`none`:** `brew bundle` 後に **自動では formula を消さない**。`wtp` のように brew 上の **`git` / `gettext` 等に依存する tap ツール**があると、`cleanup = uninstall` が依存を削ろうとして **拒否エラー**になることがある。その場合は `none` にし、不要 formula は手動で整理する。
...
- **PATH の確認:** `which -a git rg fzf` などで **`/etc/profiles/per-user/<ユーザー>/bin` が先**になることを確認する。`wtp shell-init` が PATH を後から変えるため、`programs.zsh.initContent` の末尾で Nix の `bin` を再度先頭に付け直している。
...
- **`which -a git` で `/usr/bin` や Homebrew が Nix より先:** `home.sessionPath` の先頭に `/etc/profiles/per-user/<ユーザー>/bin` などを置いている。`wtp shell-init` のあとでも Nix を先にするよう `initContent` で `PATH` を付け直している。まだ Homebrew が先なら `exec zsh` で読み直す。
- **`brew bundle` / `cleanup` が `wtp` の依存で失敗:** `onActivation.cleanup` を **`none`** にする（このリポジトリの `hosts` 設定）。`brew uninstall` で `opencode` のように **nixpkgs に移した formula が brew に残っている**と、`brew untap` が失敗することがある。その場合は `brew uninstall <formula>` してから再適用。
