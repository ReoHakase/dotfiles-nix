...
Nix 検証用 workflow を戻した場合は `actrun workflow run .github/workflows/<workflow>.yml` でローカル検証できる。現在の `.github/workflows/` は Renovate のみ。

## CI（GitHub Actions）

現在の `.github/workflows/` は Renovate のみで、Nix 構成の検証 CI は置いていない。ローカルでは次を実行する。
...
| `nix build '.#packages.x86_64-linux.home-reohakuta-kcvl' --no-link` | Linux Home Manager activation package のビルド確認。 |
...
> [!TIP] > Nix 検証 CI を戻す場合は、Apple Silicon 向け（`aarch64-darwin`）構成に合う macOS runner を選び、`darwin-rebuild switch` ではなく `nix build '.#darwinConfigurations.reohakase.system' --no-link` を実行する。
