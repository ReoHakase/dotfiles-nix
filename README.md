...
.github/workflows/nix.yml # CI（nix flake check）
...
**`nix flake check` や `darwin-rebuild` / `./scripts/apply-system.sh` の前に、必ずコミットする必要はない。**
...
- [x] GitHub Actions で `nix flake check` を実行する（`.github/workflows/nix.yml`）
...
- [ ] `brew list --formula` を見て、Nix で代替済みの formula を `brew uninstall` していく（無理なものは後回しでリスト化）— 手順は [MANUAL.md](MANUAL.md)「Homebrew」
...
- **`Existing file '…' would be clobbered`（home-manager 適用で停止）:** 手元の `~/.zshrc` や `~/.config/gh/config.yml` などが HM の管理ファイルと重なる。このリポジトリでは `hosts/reohakase.nix` の **`home-manager.backupFileExtension`** で退避してからリンクする。それでも止まる場合は [MANUAL.md](MANUAL.md) の初回手順を参照。
- **`Determinate detected, aborting activation`:** Determinate と nix-darwin の Nix 管理がぶつかっている。`hosts/<hostname>.nix` で `nix.enable = false` にする（このリポジトリでは既に設定済み）。README スタック節の「Determinate Nix を使う場合」も参照。
- **`No space left on device` でビルド失敗:** ディスク空きを増やし、`nix-collect-garbage -d` などでストアを整理する。詳細は [MANUAL.md](MANUAL.md) の「ディスク不足」。
