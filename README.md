...
- [Homebrew からの移行](#homebrew-からの移行方針メモ)
- [TODO リスト](#todo-リスト引き継ぎ用)
- [トラブル時のヒント](#トラブル時のヒント)
...
> [!NOTE]
> **なぜ `hosts/` と `home/` が分かれるか:** システム全体（ユーザー作成・defaults・Homebrew）と、ユーザーのホーム・ドットファイルの責務が違うため。概要は会話メモか [MANUAL.md](MANUAL.md) を参照。
...
> [!WARNING]
> **`Existing file … would be clobbered` で止まる**ときは、退避ファイルと HM の「どちらを正にするか」を決めてから再適用。手順は [MANUAL.md](MANUAL.md)「`~/.config` とリポジトリが食い違ったとき」。
